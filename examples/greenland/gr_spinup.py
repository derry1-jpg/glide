"""
Greenland spinup using historical SMB data from 1850-2014 before a forward simulation using SSP 5-8.5 SMB values from ISMIP7
Run from 2015-2300  
...
def get_smb_file(year):
    ...
    return SMB_DIR / (
        f"acabf_GrIS_CESM2-WACCM_{tag}_dEBM2-1000m_v1_{year}.nc"
    )


def load_smb_file(year, target_grid):

    file = get_smb_file(year)
...
while t < t_end:
    ...
    year = int(t)  # Ensure integer for filename

    # Load SMB for this year (historical or SSP585)
    smb = load_smb_file(year, target_grid)
    smb_gpu = cp.asarray(smb.values)

    # Set forcing
    mg.forcing.smb.set(smb_gpu)

# Print SMB diagnostics every 10 years
    if year % 10 == 0:
        print(f"Year {year}:")
        print(f"  SMB mean: {float(smb.mean()):.4f} m/yr")
        print(f"  SMB min:  {float(smb.min()):.4f}, max: {float(smb.max()):.4f}")
    # Advance model
    model.forward(t, dt)
